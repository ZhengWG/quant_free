    ...
    def _parse_strategy(self, text: str, stock_code: str, stock_name: str) -> Strategy:
        ...
        try:
            ...
            return Strategy(
                ...
                target_price=parsed.get("target_price") or parsed.get("targetPrice"),
                stop_loss=parsed.get("stop_loss") or parsed.get("stopLoss"),
                confidence=min(max(parsed.get("confidence", 0.7), 0), 1),
                reasoning=parsed.get("reasoning", text),
                risk_level=parsed.get("risk_level") or parsed.get("riskLevel") or "MEDIUM",
                time_horizon=parsed.get("time_horizon") or parsed.get("timeHorizon") or "短期",
